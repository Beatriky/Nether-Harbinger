...
public class GameMenu : MonoBehaviour
{
    ...
    public GameObject[] windows;    //for items and status-easier with array
    ...
    void Update()
    {
        if (Input.GetKeyDown(KeyCode.B) || Input.GetKeyDown(KeyCode.M)) // if(Input.GetButtonDown("Fire2"))

        {
            ...
            AudioManager.instance.PlaySFX(5);  //bleep
        }
    ...
    public void UpdateMainStats()
    {
        ...
        for (int i = 0; i < playerStats.Length; i++)
        {
            if (playerStats[i].gameObject.activeInHierarchy)
            {
                ...
                charImage[i].sprite = playerStats[i].charIamge;         //din charstats
            }
            else
            {
    ...
    public void DiscardItem()       //check if the item active is selected and can be discarded
    {
        if (activeItem != null)
        {
